import { age } from "@/app/config";
import { SelfBackendVerifier } from "@selfxyz/core";
import { NextResponse } from "next/server";

const MINIMUM_AGE = age;
const SERIAL_URL = process.env.SERIAL_SERVICE_URL || "http://localhost:4000";
...
export async function POST(request: Request) {
  try {
    ...
    if (!proof || !publicSignals) {
      ...
      await fetch(`${SERIAL_URL}/lock`, { method: "POST" });
    ...
    if (result.isValid) {
      ...
      // success → unlock
      await fetch(`${SERIAL_URL}/unlock`, { method: "POST" });
